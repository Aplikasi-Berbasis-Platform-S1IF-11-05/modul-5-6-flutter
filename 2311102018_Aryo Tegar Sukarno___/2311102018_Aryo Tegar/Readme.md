...
<p align="justify">
Flutter adalah framework open-source yang dikembangkan oleh Google untuk membangun aplikasi lintas platform menggunakan bahasa pemrograman Dart. Framework ini menyediakan berbagai widget seperti Column dan Row yang digunakan untuk menyusun tampilan antarmuka secara fleksibel dan terstruktur. Dalam pengembangan aplikasi modern, API (Application Programming Interface) berperan sebagai media pertukaran data antara aplikasi dan server melalui jaringan internet. Pada praktikum ini, proses pengambilan data dilakukan menggunakan metode HTTP GET dengan bantuan library http pada Flutter untuk mengambil data dari QEmail API Documentation melalui endpoint Domains. Data yang diperoleh dari server berformat JSON yang berisi atribut id dan name, kemudian dikonversi menjadi objek model di Flutter agar dapat ditampilkan secara dinamis dan real-time pada aplikasi.
</p>
...
<img src="WhatsApp Image 2026-05-26 at 21.49.05.jpeg" alt="preview" style="width:70%">

# Penjelasan

<p align="justify">
Aplikasi ini dibangun menggunakan framework Flutter dengan bahasa pemrograman Dart. Struktur utama aplikasi terdiri dari tiga bagian yaitu model, service API, dan tampilan (UI). Bagian model (Domain) digunakan untuk merepresentasikan data domain yang diterima dari API dalam bentuk objek Dart, di mana data JSON dari server diubah menjadi objek menggunakan metode fromJson agar lebih mudah dikelola di dalam aplikasi. Sementara itu, bagian service (DomainService) berfungsi sebagai penghubung antara aplikasi dan server, yang menangani proses HTTP seperti GET untuk mengambil data domain, POST untuk menambah domain, PUT untuk memperbarui data, serta DELETE untuk menghapus domain. Seluruh data dikirim dan diterima dalam format JSON melalui endpoint API QEmail.
</p>
